...
document.addEventListener('DOMContentLoaded', function () {
    ...
    buttons.forEach((btn, index) => {
        console.log(`Button ${index}:`, btn, 'Student ID:', btn.getAttribute('data-student-id'));
    });
});
...
// Test function to manually open modal (can be called from console)
function testModal(studentId) {
    console.log('Testing modal with student ID:', studentId);
    openStudentModal(studentId);
}
...
function renderStudentPerformanceChart(studentData) {
    ...
    // Only show attendance for current semester (last semester in history)
    const currentSemester = studentData.semester;
    const attendanceData = studentData.history.map((h, index) => {
        // Only show attendance for the current semester, null for others
        return h.semester === currentSemester ? h.attendance : null;
    });
